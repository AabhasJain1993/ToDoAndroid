package com.express.todoandroidapp.widget;
...
import com.express.todoandroidapp.utils.SwitchButtonColorUtils;

/**
 * Common SwitchButton
 *
 * source: https://github.com/kyleduo/SwitchButton/blob/master/SwitchButton/library/src/main/java/com/kyleduo/switchbutton/SwitchButton.java
 */
...
public class CommonSwitchButton extends CompoundButton {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = attrs == null ? null : getContext().obtainStyledAttributes(attrs, R.styleable.SwitchButtonStyle);
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.SwitchButtonStyle_kswThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.SwitchButtonStyle_kswThumbColor);
            margin = ta.getDimension(R.styleable.SwitchButtonStyle_kswThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.SwitchButtonStyle_kswThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.SwitchButtonStyle_kswThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.SwitchButtonStyle_kswThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.SwitchButtonStyle_kswThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.SwitchButtonStyle_kswThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.SwitchButtonStyle_kswThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.SwitchButtonStyle_kswThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.SwitchButtonStyle_kswBackRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.SwitchButtonStyle_kswBackDrawable);
            backColor = ta.getColorStateList(R.styleable.SwitchButtonStyle_kswBackColor);
            backMeasureRatio = ta.getFloat(R.styleable.SwitchButtonStyle_kswBackMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.SwitchButtonStyle_kswAnimationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.SwitchButtonStyle_kswFadeBack, true);
            tintColor = ta.getColor(R.styleable.SwitchButtonStyle_kswTintColor, tintColor);
            textOn = ta.getString(R.styleable.SwitchButtonStyle_kswTextOn);
            textOff = ta.getString(R.styleable.SwitchButtonStyle_kswTextOff);
            textMarginH = Math.max(textMarginH, backRadius / 2);
            textMarginH = ta.getDimension(R.styleable.SwitchButtonStyle_kswTextMarginH, textMarginH);
            autoAdjustTextPosition = ta.getBoolean(R.styleable.SwitchButtonStyle_kswAutoAdjustTextPosition, autoAdjustTextPosition);
            ta.recycle();
        }
        ...
        if (mTintColor == 0) {
            //modify by ruicb:we shouldn't support R.attr.colorAccent
        ...
        if (!mIsThumbUseDrawable && mThumbColor == null) {
            mThumbColor = SwitchButtonColorUtils.generateThumbColorWithTintColor(mTintColor);
        ...
        if (!mIsBackUseDrawable && mBackColor == null) {
            mBackColor = SwitchButtonColorUtils.generateBackColorWithTintColor(mTintColor);
